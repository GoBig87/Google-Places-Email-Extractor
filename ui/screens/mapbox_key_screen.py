    ...
    FloatLayout:
        AnchorLayout:
            ...
            MDCard:
                ...
                MDLabel:
                    size_hint: 1, 0.25
                    text: "In order to render the map provider (Mapbox) an API key is required.  API keys can be obtained from Mapbox from the link below.  Then paste the API key below."
                    font_style: "Body1"
                    theme_text_color: "Custom"
                    text_color: root.icon_color
                HyperLink:
                    size_hint_y: 0.15
                    text: "Link to Mapbox API Key"
                    font_style: "Subtitle2"
                    font_size: '14dp'
                    on_release: root.open_url(root.mapbox_url)
                RightClickTextInput:
                    id: key_input
...
class MapboxKeyScreen(Screen):
    mapbox_url = StringProperty("https://docs.mapbox.com/api/accounts/tokens/")
    ...
    def __init__(self, **kwargs):
        super(MapboxKeyScreen, self).__init__(**kwargs)
        ...
        Clock.schedule_once(self.add_corner_icons)
    ...
    def handle_textinput(self, *args):
        self.mapbox_key = self.ids.key_input.text
        if App.get_running_app().theme_cls.theme_style == "Dark":
            self.icon_name = "moon-waxing-crescent"
            self.icon_color = [1, 1, 1, 1]
        else:
    ...
    def handle_toggle(self, *args):
        ...
        self.ids.mapbox_card.update_md_bg_color(self.ids.mapbox_card, self.ids.mapbox_card.theme_cls.theme_style)
        self.ids.mapbox_card.update_md_bg_color(self.ids.mapbox_card, self.ids.mapbox_card.theme_cls.theme_style)
